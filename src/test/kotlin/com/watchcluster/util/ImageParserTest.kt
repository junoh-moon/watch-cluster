...
class ImageParserTest {
    ...
    fun `parseImageString handles all image format cases`() {
        // No registry cases
        assertEquals(ImageComponents(null, "nginx", "latest"), 
            ImageParser.parseImageString("nginx"))
        assertEquals(ImageComponents(null, "library/nginx", "latest"), 
            ImageParser.parseImageString("library/nginx"))
        
        // With tag
        assertEquals(ImageComponents(null, "nginx", "1.20.0"), 
            ImageParser.parseImageString("nginx:1.20.0"))
        
        // Registry cases
        assertEquals(ImageComponents("docker.io", "nginx", "1.20.0"), 
            ImageParser.parseImageString("docker.io/nginx:1.20.0"))
        assertEquals(ImageComponents("localhost", "myapp", "latest"), 
            ImageParser.parseImageString("localhost/myapp"))
        assertEquals(ImageComponents("registry:5000", "myapp", "latest"), 
            ImageParser.parseImageString("registry:5000/myapp"))
        
        // With digest
        assertEquals(ImageComponents("ghcr.io", "owner/repo", "latest"), 
    ...
    fun `compareVersions handles different length versions`() {
        assertTrue(ImageParser.compareVersions(listOf(2, 0, 0), listOf(1, 0, 0)) > 0)
        assertTrue(ImageParser.compareVersions(listOf(1, 0, 0), listOf(1, 1, 0)) < 0)
        assertEquals(0, ImageParser.compareVersions(listOf(1, 0, 0), listOf(1, 0, 0)))
        // Different lengths
        assertTrue(ImageParser.compareVersions(listOf(1, 0, 1), listOf(1, 0)) > 0)
        assertEquals(0, ImageParser.compareVersions(listOf(1, 0), listOf(1, 0, 0)))
    }
