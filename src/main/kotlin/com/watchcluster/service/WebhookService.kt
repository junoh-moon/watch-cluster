...
class WebhookService(
    ...
    suspend fun sendWebhook(event: WebhookEvent) {
        ...
        repeat(webhookConfig.retryCount) { attempt ->
            runCatching {
                ...
                logger.warn(
                    e,
                ) { "Webhook attempt ${attempt + 1} failed for ${event.eventType}: ${event.deployment.namespace}/${event.deployment.name}" }
    ...
    private suspend fun sendHttpRequestAsync(
        url: String,
        event: WebhookEvent,
    ): HttpResponse<String> {
